"""
Django settings for pyirkdjango.django_project.
...
INSTALLED_APPS = [
    ...
    "django_bleach",
    # the example app
    "pyirkdjango.apps.MainAppConfig",
]
...
ROOT_URLCONF = "pyirkdjango.django_project.urls"
...
WSGI_APPLICATION = "pyirkdjango.django_project.wsgi.application"
...
class LazyContainer:
    """
    some variables should be easily available as attribute of settings, however they depend on pyirk
    and during unittesting that module can only be loaded after os.environ["PYIRK_BASE_DIR"] has been set.
    ...
    def __getattr__(self, attrname: str):
        ...
        if not self.store["initialized"]:
            msg = "initialization of pyirk settings is incomplete"
    ...
    def initialize_pyirk_settings(self):
        import pyirk.settings
        self.store["DEFAULT_DATA_LANGUAGE"] = pyirk.settings.DEFAULT_DATA_LANGUAGE

        conf = pyirk.settings.CONF
        if not conf:
            msg = "`pyirk.settings.CONF` is empty, probably because env variable PYIRK_BASE_DIR is empty or wrong"
            raise FileNotFoundError(msg)
        main_mod = conf["main_module"]

        self.store["IRK_DATA_MAIN_MOD_NAME"] = main_mod.rstrip(".py")
        self.store["IRK_DATA_MAIN_MOD"] = os.path.join(pyirk.settings.BASE_DIR, main_mod)
        self.store["IRK_DATA_MAIN_MOD_PREFIX"] = conf["main_module_prefix"]
        self.store["IRK_DATA_BASE_DIR"] = pyirk.settings.BASE_DIR
